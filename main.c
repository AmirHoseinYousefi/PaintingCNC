...
**/
/**
	stm32f7xx_ll_sdmmc.c line 318 changed to ->
...
#define ProcessSize  			   25000
#define ReadBuffSize         25000
#define NumberOfFirstRead    2
/* USER CODE END PD */
...
uint8_t Buff[BuffSize] = {5};
uint8_t Process[ProcessSize] = {5};
uint8_t ReadBuff[ReadBuffSize] = {5};
uint8_t GetFileSize[NumberOfFirstRead] = {5};
  ...
  while (1)
	{
		if (ProcessingFile)
		{
			FR1 = f_mount(&MyFatfs, SDPath, 0);
			
			FR2 = f_open(&MyFile1, "MAPCODE.txt", FA_READ | FA_OPEN_ALWAYS);
    	FR3 = f_open(&MyFile2, "MAPDECODE.txt", FA_WRITE | FA_CREATE_ALWAYS);
	    ...
	    f_read(&MyFile1, GetFileSize, sizeof(GetFileSize), &ByteCount);
	    OFS1 += NumberOfFirstRead;
	
	    GetFileSize[0] -= 48;
	    GetFileSize[1] -= 48;
	    EndReadStep = 10 * GetFileSize[0] + GetFileSize[1] - 1;
			EndLoopStep = EndReadStep;
	
    	for (uint8_t ReadStep = 0; ReadStep <= EndReadStep; ReadStep++)
    	{
	      ...
	      for (ProcessStep = 1; ProcessStep <= ProcessSize; ProcessStep++)
	      {
		...
		if (Start)
		{
			FR3 = f_open(&MyFile2, "MAPDECODE.txt", FA_READ | FA_OPEN_ALWAYS);
	    OFS2 = 0;
			
		  for (uint8_t LoopStep = 0; LoopStep <= EndLoopStep; LoopStep++)
		  {
			  f_lseek(&MyFile2, OFS2);
		  	f_read(&MyFile2, ReadBuff, sizeof(ReadBuff), &ByteCount);
		  	OFS2 += ReadBuffSize;
			
		  	for (uint16_t i = 0; i <= 24999; i++)
			  {
		      switch (ReadBuff[i])
          {
            case 1:
